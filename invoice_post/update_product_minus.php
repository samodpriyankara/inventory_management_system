<?php
    ...
    if($_POST)
    {
        ...
        $ProductQuantityMinussql = "UPDATE tbl_order_item_details SET qty=qty-1 WHERE order_id='$invoice_id' AND id='$product_id'";

        if ($conn->query($ProductQuantityMinussql) === TRUE) 
        {
            // echo 'Completed';
            ...
            if ($conn->query($ItemQuantitySql) === TRUE) 
            {
                ...
                $checkQty = $conn->query("SELECT qty FROM tbl_order_item_details WHERE id='$product_id'");
                if($qRS = $checkQty->fetch_array()){
                    ...
                    if($cqty == 0){
                        ...
                        //Delete
                        $conn->query("DELETE FROM tbl_order_item_details WHERE id='$product_id'");
