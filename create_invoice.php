...
<style>
    ...
    .modal-body.fullscreen-body {
        overflow-y: auto;
        height: calc(100% - 56px); /* Adjust height considering modal header and footer */
    }

    .modal-header.fullscreen-header {
        padding: 10px;
        border-bottom: 1px solid #ddd;
        background-color: #f7f7f7;
    }

    .modal-footer.fullscreen-footer {
        padding: 10px;
...
<div class="modal fade" id="modal-preview-invoice">
            ...
            <div class="modal-body fullscreen-body">
                <table class="table table-striped" id="InvoiceTable">
                    ...
                    <tbody id="invoice-preview-body">
                        <!-- Your invoice data rows will be dynamically added here -->
                    </tbody>
                </table>
            </div>

            <!-- Modal footer -->
            <div class="modal-footer fullscreen-footer">
                <h4 class="pull-right" id="lbl_invoice_total1" style="padding: 10px; font-weight: 600; font-size: 30px; color: red; margin-right: 50px;">LKR 0.00</h4>
...
<!----------->
    ...
    function addToCart(input,label,item_id,price,stockQty,des,fixedDiscount,discount_margin){
      ...
      if(parseInt(qty.val()) > parseInt(stockQty)){
        ...
        var added_qty = parseInt(qty.val());
        var discount_percentage = parseFloat(fixedDiscount);

        if((added_qty > 0 && added_qty <= discount_margin) || (added_qty > 0 && 0 == discount_margin )){

            var obj = {item_id:item_id,item_qty:added_qty,price:price,name:des,discount:discount_percentage};
      // obj['item_id'] = item_id;
      ...
      $("#"+input).val("");
             ...
             var obj = {item_id:item_id,item_qty:qty.val(),price:price,name:des,discount:0};
      // obj['item_id'] = item_id;
      // obj['item_qty'] = qty.val();
      ...
      /////view the total of invoice///////
            ...
            var invoice_total1 = 0.00;
             ...
             for(var i = 0;i<invoiceItemsDetails.length;i++){
                  ...
                  invoice_total1 += parseFloat(price)*(100-po.discount)/100 * parseFloat(qty);
                ...
                $("#lbl_invoice_total").html("Total = LKR "+invoice_total1.toFixed(2));
                $("#lbl_invoice_total1").html("Total = LKR "+invoice_total1.toFixed(2));
    ...
    function check_key_up(e,input_id,lbl_id,item_id,price,stock_qty,des,category_discount,discount_margin){


        if(e.keyCode == 13){
            addToCart(input_id,lbl_id,item_id,price,stock_qty,des,category_discount,discount_margin);
        }
    }
            ...
            if(invoiceItemsDetails.length != 0){
                ...
                for(var i = 0;i<invoiceItemsDetails.length;i++){
                  ...
                  console.log(po);
                   ...
                   var name = po.name;
                  var qty = po.item_qty;
                  var total = parseFloat(po.price)*(100-po.discount)/100 * parseFloat(qty);
                  var discount_percentage = po.discount+"%";
                  var unit_price = parseFloat(po.price);

                  
                  var row = '<tr><td>'+name+'</td><td>'+qty+'</td><td>'+unit_price.toFixed(2)+'</td><td>'+discount_percentage+'</td><td>'+total.toFixed(2)+'</td><tr>';
