  <?php
    ...
    if(isset($_POST['selling_summery_distributor_id']))
    {
        $selling_summery_distributor_id = htmlspecialchars($_POST['selling_summery_distributor_id']);

        $query="SELECT itemId, SUM(toid.qty) FROM tbl_order_item_details toid INNER JOIN tbl_order tor ON toid.order_id=tor.id WHERE tor.distributor_id='$selling_summery_distributor_id' GROUP BY toid.itemId ORDER BY SUM(toid.qty) DESC";
