...
if(isset($_POST['username']) && isset($_POST['password'])){
	...
	// $rs=$conn->query("SELECT * FROM tbl_user tu INNER JOIN tbl_user_has_routes tuhr ON tu.id=tuhr.user_id INNER JOIN tbl_route tr ON tuhr.route_id=tr.route_id WHERE username='$username' AND password='$password' AND tuhr.date='$date'");
	...
	if($row=$rs->fetch_array()){
		...
		//last sales return id

		// $getLastOrder=$conn->query("SELECT order_id FROM tbl_return_order WHERE user_id='$userId' AND return_type = 0 ORDER BY timestamp DESC LIMIT 1");
		// if($gls=$getLastOrder->fetch_array()){
		// 		$output["last_sreturn_id"]=$gls[0];
		// }else{
		...
		//last damage return id

		// $getLastOrder=$conn->query("SELECT order_id FROM tbl_return_order WHERE user_id='$userId' AND return_type = 1 ORDER BY timestamp DESC LIMIT 1");
		// if($gls=$getLastOrder->fetch_array()){
		// 		$output["last_dreturn_id"]=$gls[0];
		// }else{
